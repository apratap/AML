...
synapseLogin()
query_df <- synQuery("select * from entity where parentId == 'syn2355194'")
...
patientIds <- unlist(lapply(colnames(miRNA_exp_matrix), function(x){ patientID <- query_df[query_df$name == paste0(x,'.gpr'),]$PatientID}))
...
syn_miRNA_files <- synapseQuery('SELECT id, name FROM entity WHERE parentId=="syn2355194"')
...
miRNA_arrayQC1 <- File("~/dev/AML/dset1_miRNA_expression_raw_intensities(log2).jpg",synapseStore=T,parentId ="syn2355193")
miRNA_arrayQC1 <- synStore(miRNA_arrayQC1, activity=act)

#push the miRNA raw exp vals to synapse
miRNA_expVals <- File(miRNA_exp_matrix_file, synapseStore=T, parentId = "syn2355193")
